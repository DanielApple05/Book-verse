...
const Categories = () => {
  ...
  return (
    <div className='my-6 dark:text-gray-300 text-black'>
      ...
      <div className='flex py-2 gap-3 w-full overflow-x-auto '>
        {categoryTabs.map((tab, index) => (
          <div
            ...
            key={tab.id}
            className='flex flex-col items-center gap-2 p-4 rounded-xl border border-gray-100 dark:bg-gray-800 bg-white shadow-sm hover:shadow-md hover:-translate-y-1 transition-all cursor-pointer min-w-24'
          >
            ...
            <span className='text-xs font-semibold capitalize'>{tab.tab}</span>
          </div>
        ))}
      </div>
    </div>
